...
# 데이터 어그멘테이션을 위한 ImageDataGenerator 생성
datagen = ImageDataGenerator(
    rotation_range=20,  # 랜덤한 각도로 이미지 회전 (0 ~ 20도)
    width_shift_range=0.2,  # 이미지 가로로 랜덤 이동 (전체 너비의 20% 범위)
    height_shift_range=0.2,  # 이미지 세로로 랜덤 이동 (전체 높이의 20% 범위)
    shear_range=0.2,  # 랜덤한 각도로 전단 변형
    zoom_range=0.2,  # 랜덤한 범위로 이미지 확대/축소
    horizontal_flip=True,  # 랜덤하게 이미지 좌우 반전
    vertical_flip=True,
    fill_mode='nearest'  # 이미지 변환 시 새로 생성된 픽셀 채우는 방식
)

# 모델 훈련
history = model.fit(
    datagen.flow(x_train, y_train, batch_size=32),  # 데이터 어그멘테이션 적용
    steps_per_epoch=len(x_train) // 32,  # 한 에폭 당 스텝 수
    epochs=250,
    validation_data=(x_val, y_val)
)

model.save('C:\\딥러닝\\inceptionv3_model7_batch_32_aug.h5')
